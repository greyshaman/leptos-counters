...
const MANY_COUNTERS: usize = 20;

type CountersHolder = Vec<(usize, ArcRwSignal<i32>)>;

#[derive(Debug, Copy, Clone)]
struct CounterUpdater {
    set_counters: WriteSignal<CountersHolder>,
}

#[component]
pub fn Counters() -> impl IntoView {
    let (next_counter_id, set_next_counter_id) = signal(0);
    let (counters, set_counters) = signal::<CountersHolder>(vec![]);
    ...
    let add_many_counters = move |_| {
        ...
        let new_counters = (next_id..next_id + MANY_COUNTERS)
            .map(|id| {
                let sig = ArcRwSignal::new(0);
                (id, sig)
            });
